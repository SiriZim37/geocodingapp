...
import com.tlt.georepo.model.request.JobOnHandDetailRequest
import com.tlt.georepo.model.response.ItemJobOnHandDetailResponse
import com.tlt.georepo.model.response.ItemJobOnhandResponse
import com.tlt.georepo.modules.menu.job.onhand.common.*
...
class SubDetailMainViewModel : ViewModel() {
    ...
    private var job_Detaillist : List<OnHandDetailItem>? = null
    private var job_Remarklist : List<OnHandRemarkItem>? = null
    private var job_Profilelist : List<OnHandProfileItem>? = null
    ...
    fun getData(mainIDExtra : String , lat: String , lng: String) {
        GlobalScope.launch(Dispatchers.Main) {
            ...
            try {
                val model = getJobOnHandDetail(mainIDExtra , lat , lng)
    ...
    private suspend fun getJobOnHandDetail(mainIDExtra : String , lat : String , lng : String ) = suspendCoroutine<ItemJobOnhandResponse?> {
        val request = JobOnHandDetailRequest.build(mainIDExtra , lat , lng)

        apiManager.getJobOnHandDetail(request){ isError: Boolean, result: String ->
            try{
                whenLoading.value = false
                if (isError) {
                    it.resumeWithException(Exception(result))
                    return@getJobOnHandDetail
                }
                try {

                    val items = JsonMapperManager.getInstance()
                        .gson.fromJson(result, Array<ItemJobOnHandDetailResponse>::class.java)
                ...
                Log.e( "getJobOnHandDetail : " , e.stackTrace.toString() )
                whenLoading.postValue(false)
            }
        }
    ...
    data class Model(
        var jobDetailList: List<OnHandDetailItem> = listOf(),
        var jobRemarkList: List<OnHandRemarkItem> = listOf(),
        var jobProfileList: List<OnHandProfileItem> = listOf()
    )
