...
import com.tlt.georepo.modules.menu.job.request.detail.common.RequestProfileGuarantorItem
...
class RequestMainDetailViewModel : ViewModel() {
    ...
    val whenSendToApiLimit = MutableLiveData<String>()
    val whenSendToApiSuccess = MutableLiveData<String>()
    val whenDataLoadedLocationSuccess = MutableLiveData<LatLng>()
    private var job_Detaillist : List<RequestDetailItem>? = null
    private var job_Remarklist : List<RequestRemarkItem>? = null
    private var job_Profilelist : List<RequestProfileItem>? = null
    ...
    data class Model(
        var jobDetailList: List<RequestDetailItem> = listOf(),
        var jobRemarkList: List<RequestRemarkItem> = listOf(),
        var jobProfileList: List<RequestProfileItem> = listOf()
    )
            ...
            try {
                ...
                apiManager.setActionJob(request) { isError: Boolean, result: String ->
                    ...
                    if(result.equals("limit")){
                        whenSendToApiLimit.postValue(result)
                        return@setActionJob
                    }
