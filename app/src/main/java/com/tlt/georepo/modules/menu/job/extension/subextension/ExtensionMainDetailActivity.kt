...
class ExtensionMainDetailActivity : BaseActivity() {

    private var CRRENT_LAT  : String = ""
    private var CRRENT_LNG : String = ""

    private val viewModel by lazy {
        ViewModelProviders.of(this).get(ExtensionMainDetailViewModel::class.java)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.extension_main_detail_activity)
...
//                setupDataIntoViews(it)
                jobExtensionDetailListData = it.jobDetailList
                jobExtensionRemarkListData = it.jobRemarkList
                jobExtensionProfileListData = it.jobProfileList
                initTabs()
            }
...
//        initTabs()
    ...
    private fun initTabs( ) {
        ...
        fragmentList.add(findFragment(ExtensionMainFragment::class.java)
                ?: ExtensionMainFragment.newInstance())
        fragmentList.add(findFragment(ExtensionDetailFragment::class.java)
                ?: ExtensionDetailFragment.newInstance())
        fragmentList.add(findFragment(ExtensionRemarkFragment::class.java)
                ?: ExtensionRemarkFragment.newInstance())
    ...
    fun OnClickBacktoLastActivity(){
        try {
            JobExtensionMainMapActivity.Open(this)
        }catch (e : Exception){
            e.message
        }
    }
    ...
    companion object {
        var jobExtensionDetailListData : List<ExtensionDetailItem>? = null
        var jobExtensionRemarkListData : List<ExtensionRemarkItem>? = null
        var jobExtensionProfileListData :  List<ExtensionProfileItem>? = null
        ...
        fun start(context: Context?,
            ...
            val intent = Intent(context, ExtensionMainDetailActivity::class.java).apply {
